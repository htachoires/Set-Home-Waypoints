...
public class HomeCommand {
    ...
    private static int goHome(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (currentHome == null) {
            context.getSource().sendFailure(new TranslatableComponent("shw.commands.home.error.homeNotFound", CommandLineFormatter.formatCommand(SetHomeCommand.COMMAND_NAME)));
        ...
        if (!ShwConfigWrapper.isDimensionalTravelAllowedForHome() &&
            ...
            context.getSource().sendFailure(new TranslatableComponent("shw.commands.home.error.dimensionalTravelNotAllowed"));
        ...
        if (cooldownRemaining > 0) {
            context.getSource().sendFailure(new TranslatableComponent("shw.commands.home.error.cooldown", TimeUnit.MILLISECONDS.toSeconds(cooldownRemaining) + 1));
        ...
        context.getSource().sendSuccess(new TranslatableComponent("shw.commands.home.success"), false);
    ...
    private static int showConfiguration(CommandContext<CommandSourceStack> context) {
        context.getSource().sendSuccess(
                new TranslatableComponent("shw.commands.home.config.success",
                        new TextComponent(String.valueOf(ShwConfigWrapper.getHomeCooldown())).withStyle(ChatFormatting.BLUE),
    ...
    private static int configureCooldown(CommandContext<CommandSourceStack> context) {
        ...
        for (ServerPlayer player : context.getSource().getServer().getPlayerList().getPlayers()) {
            player.sendMessage(new TranslatableComponent("shw.commands.home.config.cooldown.success", CommandLineFormatter.formatCommand(COMMAND_NAME), new TextComponent(String.valueOf(cooldownValue))).withStyle(ChatFormatting.GRAY), player.getUUID());
        }
    ...
    private static int configureDimensionalTravel(CommandContext<CommandSourceStack> context) {
        ...
        for (ServerPlayer player : context.getSource().getServer().getPlayerList().getPlayers()) {
            player.sendMessage(new TranslatableComponent("shw.commands.home.config.dimensionalTravel.success", CommandLineFormatter.formatCommand(COMMAND_NAME), new TextComponent(String.valueOf(dimensionalTravel))).withStyle(ChatFormatting.GRAY), player.getUUID());
        }
