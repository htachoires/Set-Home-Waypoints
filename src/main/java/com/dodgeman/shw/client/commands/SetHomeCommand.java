...
public class SetHomeCommand {
    ...
    private static int setHome(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (newHome.position().isInTheNether() && !playerHomeAndWaypoints.hasAlreadySetHomeInTheNether()) {
            successMessage = new TranslatableComponent("shw.commands.sethome.success.the_nether", new TextComponent("The Nether").withStyle(ChatFormatting.DARK_PURPLE)).withStyle(ChatFormatting.GREEN);
        }

        if (newHome.position().isInTheEnd() && !playerHomeAndWaypoints.hasAlreadySetHomeInTheEnd()) {
            successMessage = new TranslatableComponent("shw.commands.sethome.success.the_end", new TextComponent("The End").withStyle(ChatFormatting.DARK_PURPLE)).withStyle(ChatFormatting.GREEN);
        }

        if (currentHome == null) {
            successMessage = new TranslatableComponent("shw.commands.sethome.success.first_home", formatCommand(HomeCommand.COMMAND_NAME)).withStyle(ChatFormatting.GREEN);
        }

        if (successMessage == null) {
            successMessage = new TranslatableComponent("shw.commands.sethome.success.update").withStyle(ChatFormatting.GREEN);
        }
