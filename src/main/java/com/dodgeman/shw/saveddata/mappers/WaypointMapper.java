...
public class WaypointMapper implements CompoundMapper<Waypoint> {
    ...
    @Override
    public Waypoint fromCompoundTag(NBTTagCompound tag) {
    ...
    @Override
    public NBTTagCompound toCompoundTag(Waypoint waypoint) {
        NBTTagCompound tag = new NBTTagCompound();
        if (waypoint == null) return tag;

        tag.setString(NAME_KEY, waypoint.name().value());
        tag.setTag(POSITION_KEY, positionCompoundMapper.toCompoundTag(waypoint.position()));
